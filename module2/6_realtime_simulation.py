...
# Run from project root so module2 and config are findable
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from module2 import config
from module2 import safety
...
def run_simulation(max_steps=200, verbose=True):
    ...
    # Pre-build sequences from full dataset for LSTM (so we have history per row)
    ...
    for i in range(config.SEQ_LENGTH, config.SEQ_LENGTH + max_steps):
        ...
        # Build sequence from last SEQ_LENGTH rows
